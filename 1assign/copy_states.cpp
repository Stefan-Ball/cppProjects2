...
state *read_in_data(arguments args) {
   ...
   for (int i = 0; i < args.num_states; i++) {
      ...
      get_county_data(state_data[i].c, state_data[i].counties, input);
...
void get_county_data(county *county_data, int num_counties, ifstream &input) {
   ...
   //Get the county data for the state
   for (int i = 0; i < num_counties; i++) {

      //Create county for state
      county_data = create_counties(num_counties);

      //Get the name of the county in the state
      input >> county_data[i].name;

      cout << "Print county name" << endl;
      cout << county_data[i].name << endl;

      //Get the population of the county
      input >> county_data[i].population;

      //Get the average income of the county
      input >> county_data[i].avg_income;

      //Get the average household price for the county
      input >> county_data[i].avg_house;

      //Get the number of cities in the county
      input >> county_data[i].cities;

      cout << "Print number of cities in county" << endl;
      cout << county_data[i].cities << endl;

      get_city_data(county_data, i, county_data[i].cities, input);
   }
