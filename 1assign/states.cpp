...
void run_program(int argc, char *argv[]) {
   ...
   //cout << state_data[1].c[0].city[0] << endl;
...
void state_with_largest_population(state *, int num_states, ofstream &) {
...
/*
   void states_in_order_by_name(state *state_data, int num_states, ofstream &) {

   int state_order = new int[num_states];
...
//for (int k = 0; k < state_data[j].name.at(k).length(); k++) {
